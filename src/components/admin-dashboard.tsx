...
import { User, Users, Vault, Settings, CheckCircle, RefreshCw, X, Plus, XCircle, Loader2 } from "lucide-react"
...
import { AdminTable, Button, Card, CardHeader, CardTitle, CardContent, Input, Label } from "@/components/admin-table"
...
export default function AdminDashboard() {
  ...
  const { data: fundsCycleData, isLoading: fundsCycleLoading, error: fundsCycleError, refetch } = useFundsCycleData()
  const { data: fundsCycleExists } = useFundsCycleExists()
  ...
  const punishMutation = usePunishMutation()
  ...
  // Check if current user is admin (first beneficiary) or if they're in the beneficiaries list
  const currentUserBeneficiary = beneficiaries.find(b => b.wallet === account?.address)
  const isAdmin = currentUserBeneficiary?.index === 0 // Admin is typically at index 0
  
  const handleAddBeneficiary = async () => {
    if (!beneficiaryAddress.trim()) return
    
    try {
      await addBeneficiaryMutation.mutateAsync({ 
        wallet: beneficiaryAddress.trim() as Address // Type assertion needed based on your Address type
      })
  ...
  const handlePunish = async (beneficiaryWallet: string) => {
    if (!isAdmin) {
      toast.error("Only the admin can punish beneficiaries")
      return
    }

    if (!confirm(`Are you sure you want to punish beneficiary ${beneficiaryWallet.slice(0, 8)}...${beneficiaryWallet.slice(-8)}? This action cannot be undone.`)) {
      return
    }

    try {
      await punishMutation.mutateAsync({ 
        beneficiaryAddress: beneficiaryWallet as Address 
      })
      toast.success("Beneficiary punished successfully")
    } catch (error) {
      console.error("Failed to punish beneficiary:", error)
      toast.error("Failed to punish beneficiary")
    }
  }

  const handleExit = async() => {
    if (!isAdmin) {
    toast.error("Only the admin can exit the program")
    return
  }

  // Confirmation dialog would be good here since this is destructive
  if (!confirm("Are you sure you want to exit the program? This will close the cycle and return remaining funds to the admin.")) {
    return
  }
 ...
 try {
     ...
     toast.error("Failed to exit program")
   }
  }
        ...
        <div className="container mx-auto p-6 max-w-7xl relative z-10">
          <div className="flex items-center justify-center min-h-[400px]">
            <div className="flex flex-col items-center gap-4 text-center">
              ...
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white">No FundsCycle Found</h2>
              <p className="text-gray-500 max-w-md">
                You have not created a FundsCycle yet. Initialize one to start managing your group savings program.
              </p>
            </div>
          </div>
        </div>
...
>
  ...
  Refresh Status
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* Add Beneficiary */}
          <Card className="shadow-xl bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm border-2 border-white/20 dark:border-gray-800">
            <CardHeader>
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 flex items-center justify-center rounded-xl bg-gradient-to-r from-blue-100 to-purple-100 dark:from-blue-900/30 dark:to-purple-900/30">
                  <Plus className="w-5 h-5 text-blue-600 dark:text-blue-400" />
                </div>
                <CardTitle className="text-xl">Add Beneficiary</CardTitle>
              </div>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                <Label className="text-sm text-gray-900 dark:text-gray-100 mb-2">
                  Wallet Address
                </Label>
                <Input
                  value={beneficiaryAddress}
                  onChange={(e: any) => setBeneficiaryAddress(e.target.value)}
                  placeholder="Enter wallet address..."
                  className="bg-white/50 dark:bg-gray-900/80 backdrop-blur-sm border border-gray-200 focus:border-blue-300 transition-all duration-300"
                />
              </div>
              <Button
                ...
                disabled={
                  ...
                  beneficiaries.length >= MAX_PARTICIPANTS ||
              ...
              <div className="text-center">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-gradient-to-r from-blue-100 to-purple-100 border border-blue-200">
                  <div className="w-2 h-2 rounded-full bg-green-500" />
                  <span className="text-sm font-medium text-blue-800">
                    Available slots: {MAX_PARTICIPANTS - beneficiaries.length}
                  </span>
                </div>
              </div>
            </CardContent>
        ...
        {/* Beneficiaries Table - Only shows when there are beneficiaries */}
        <AdminTable
          beneficiaries={beneficiaries}
          isAdmin={isAdmin}
          handlePunish={handlePunish}
          punishMutation={punishMutation}
        />
